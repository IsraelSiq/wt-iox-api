...
app = FastAPI(
    title="wt-iox-api",
    description="War Thunder IOX API — HTTP poller + REST/WebSocket server",
    version="0.1.0",
    lifespan=lifespan,
)
...
async def get_state():
    if shared.latest_state is None:
        raise HTTPException(status_code=503, detail="No data from War Thunder yet.")
...
async def get_telemetry():
    ...
    return {
        ...
        "speed":     {"ias_ms": s.ias_ms, "ias_kts": round(s.ias_ms * 1.944, 1),
                      "tas_ms": s.tas_ms, "mach": s.mach, "vvi_ms": s.vvi_ms},
        "attitude":  {"heading_deg": s.heading_deg, "pitch_deg": s.pitch_deg,
                      "bank_deg": s.bank_deg, "aoa_deg": s.aoa_deg},
    }
...
<style>
  :root{--bg:#0a0c0e;--surface:#0f1215;--border:#1e2428;--text:#c8d8e8;--text-muted:#5a7a8a;--primary:#40c8ff;--radius:6px;--font:'Fira Code','Cascadia Code','Consolas',monospace;}
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .btn{padding:4px 12px;background:var(--primary);color:#0a0c0e;border:none;border-radius:var(--radius);cursor:pointer;font-size:12px;font-weight:600;font-family:var(--font);transition:opacity .15s}
...
<header>
  <div class="logo"><h1>wt-iox-api &mdash; Live Logs</h1></div>
  <div style="display:flex;gap:8px;align-items:center">
    <div class="badge"><div class="dot"></div><span id="status-text">conectando...</span></div>
    <span id="countdown" class="badge">next: 10s</span>
  </div>
</header>
<div class="toolbar">
  ...
  <button class="btn" onclick="clearView()" style="background:#1e2428;color:var(--text)">Limpar</button>
</div>
...
<script>
  ...
  function resetCountdown(){
    ...
    timer=setInterval(()=>{countdown--;document.getElementById('countdown').textContent='next: '+countdown+'s';if(countdown<=0)fetchLogs();},1000);
  }
  fetchLogs();
</script></body></html>"""
...
<style>
  ...
  :root{--bg:#080a10;--panel:#0c0f18;--border:#182030;--green:#40c8ff;--green-dim:#0a2a40;--amber:#ffb830;--red:#ff4040;--blue:#40c8ff;--text:#b0c8e8;--muted:#3a5a7a;--font-mono:'Share Tech Mono',monospace;--font-hud:'Orbitron',sans-serif;--glow:0 0 8px rgba(64,200,255,0.35);}
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  #heading-tape-wrap{width:300px;height:36px;background:#080a10;border:1px solid var(--border);border-radius:4px;overflow:hidden;position:relative}
  ...
  #offline{display:none;position:fixed;inset:0;background:rgba(8,10,16,.88);z-index:100;align-items:center;justify-content:center;flex-direction:column;gap:16px;font-family:var(--font-hud);color:var(--red);font-size:18px;letter-spacing:.1em;text-align:center}
...
<div id="main">
  ...
  <div class="panel">
    ...
    <div class="divider"></div>
    <div class="gauge-block"><div class="gauge-label">ENGINE 1</div><div class="gauge-value" id="rpm-val">—</div><div class="gauge-unit">%</div></div>
  </div>
  <div id="bottombar">
...
function drawADI(pitch,bank){
  ...
  skyGrad.addColorStop(0,'#050a1a');skyGrad.addColorStop(1,'#0a1a3a');
  ...
  gndGrad.addColorStop(0,'#1a1005');gndGrad.addColorStop(1,'#0e0a04');
  ...
  ctx.restore();
  ctx.beginPath();ctx.arc(ADI_CX,ADI_CY,ADI_R,0,Math.PI*2);ctx.strokeStyle='#182030';ctx.lineWidth=3;ctx.stroke();
  ...
  ctx.rotate(bank*Math.PI/180);ctx.fillStyle='#40c8ff';ctx.beginPath();ctx.moveTo(0,-(ADI_R-14));ctx.lineTo(-5,-(ADI_R-4));ctx.lineTo(0,2);ctx.lineTo(5,-(ADI_R-4));ctx.closePath();ctx.fill();
  ctx.restore();
...
function drawHeadingTape(hdg){
  const ctx=hdgCtx,W=300,H=36;ctx.clearRect(0,0,W,H);ctx.fillStyle='#080a10';ctx.fillRect(0,0,W,H);
  ...
  for(let d=-30;d<=30;d++){const deg=((hdg+d)%360+360)%360;const x=halfW+d*pxPerDeg;
    if(deg%10===0){ctx.fillStyle='#3a5a7a';ctx.fillRect(x-.5,0,1,12);const label=deg===0?'N':deg===90?'E':deg===180?'S':deg===270?'W':deg.toString();ctx.fillStyle=(deg%90===0)?'#40c8ff':'#5a8aaa';ctx.fillText(label,x,26);}
    else if(deg%5===0){ctx.fillStyle='#1a2a3a';ctx.fillRect(x-.5,0,1,6);}}
...
function updateHUD(s){
  ...
  const g=s.g_load!=null?s.g_load.toFixed(1):'—';setVal('g-val',g);
  const altFt=Math.round((s.alt_msl_m||0)*3.28084);
  setVal('alt-val',altFt.toLocaleString());setBar('alt-bar',altFt/50000*100,70,90);
  setColor('alt-val',altFt<500?'var(--red)':altFt<1000?'var(--amber)':'var(--green)');
  if(s.fuel_kg!=null){setVal('fuel-val',Math.round(s.fuel_kg).toLocaleString());setBar('fuel-bar',50,30,15);}
  if(s.rpm_1!=null)setVal('rpm-val',s.rpm_1.toFixed(0));
...
<style>
  @import url('https://fonts.googleapis.com/css2?family=Share+Tech+Mono&family=Orbitron:wght@400;700&display=swap');
  :root{
    --bg:#05080a;--panel:#080b10;--border:#101820;
    --green:#40c8ff;--green-dim:#0a1a2a;--green-mid:#1a5a7a;
    --amber:#ffb830;--red:#ff4040;--cyan:#00e5ff;
    --neutral:#aaaaaa;--text:#a0c0d8;--muted:#2a4a5a;
    --font:'Share Tech Mono',monospace;--hud:'Orbitron',sans-serif;
    --glow:0 0 10px rgba(64,200,255,0.4);
  }
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .contact-row:hover{background:rgba(64,200,255,.06);border-color:var(--border)}
  .contact-row.selected{background:rgba(64,200,255,.1);border-color:var(--green-mid)}
  ...
  .range-btn.active{border-color:var(--green);color:var(--green);background:rgba(64,200,255,.08)}
  #detail-panel{display:none;background:rgba(64,200,255,.04);border:1px solid var(--green-mid);border-radius:4px;padding:10px 12px;margin-top:8px}
...
<div id="content">
  ...
  <div id="sidebar">
    ...
    <div class="side-section">
      ...
      <div class="legend">
        <div class="legend-item"><div class="legend-dot" style="background:#40c8ff"></div>Friendly (Allies)</div>
...
const trails={},TRAIL_MAX=8,TRAIL_TTL=15000;
...
function updateAltFilter(){altMinFt=parseInt(document.getElementById('alt-min').value);altMaxFt=parseInt(document.getElementById('alt-max').value);if(altMinFt>altMaxFt){altMaxFt=altMinFt;document.getElementById('alt-max').value=altMinFt;}document.getElementById('alt-min-lbl').textContent=altMinFt.toLocaleString();document.getElementById('alt-max-lbl').textContent=altMaxFt.toLocaleString();}
function iffColor(coal){if(coal===1)return '#40c8ff';if(coal===2)return '#ff4040';return '#ffb830';}
...
function draw(now){
  ...
  const bgGrad=ctx.createRadialGradient(CX,CY,0,CX,CY,R);bgGrad.addColorStop(0,'#060a10');bgGrad.addColorStop(1,'#020408');
  ctx.fillStyle=bgGrad;ctx.beginPath();ctx.arc(CX,CY,R,0,Math.PI*2);ctx.fill();
  ctx.strokeStyle='rgba(64,200,255,0.07)';ctx.lineWidth=1;
  for(let i=1;i<=4;i++){ctx.beginPath();ctx.arc(CX,CY,R*i/4,0,Math.PI*2);ctx.stroke();}
  ctx.strokeStyle='rgba(64,200,255,0.04)';
  ...
  ctx.fillStyle='rgba(64,200,255,0.25)';ctx.font='9px Share Tech Mono';ctx.textAlign='center';
  for(let i=1;i<=4;i++){const km=Math.round(radarRange/1000*i/4);ctx.fillText(km+'km',CX,CY-R*i/4+3);}
  // Cardinals
  ctx.font='11px Orbitron';const cards=[['N',0],['E',90],['S',180],['W',270]];
  cards.forEach(([d,a])=>{const rad=a*Math.PI/180;ctx.fillStyle='rgba(64,200,255,0.55)';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(d,CX+Math.sin(rad)*(R-14),CY-Math.cos(rad)*(R-14));});
  ctx.strokeStyle='rgba(64,200,255,0.12)';ctx.lineWidth=1;ctx.textBaseline='alphabetic';
  for(let a=0;a<360;a+=10){const rad=a*Math.PI/180;const inner=a%30===0?R-12:R-8;ctx.beginPath();ctx.moveTo(CX+Math.sin(rad)*inner,CY-Math.cos(rad)*inner);ctx.lineTo(CX+Math.sin(rad)*(R-2),CY-Math.cos(rad)*(R-2));ctx.stroke();}
  ctx.save();ctx.beginPath();ctx.arc(CX,CY,R,0,Math.PI*2);ctx.clip();
  const sweepRad=sweepAngle*Math.PI/180;const SWEEP_ARC=25*Math.PI/180;const startRad=sweepRad-SWEEP_ARC;
  const sweepFill=ctx.createLinearGradient(CX+Math.sin(startRad)*R,CY-Math.cos(startRad)*R,CX+Math.sin(sweepRad)*R,CY-Math.cos(sweepRad)*R);
  sweepFill.addColorStop(0,'rgba(64,200,255,0)');sweepFill.addColorStop(1,'rgba(64,200,255,0.12)');
  ctx.fillStyle=sweepFill;ctx.beginPath();ctx.moveTo(CX,CY);ctx.arc(CX,CY,R,startRad-Math.PI/2,sweepRad-Math.PI/2);ctx.closePath();ctx.fill();
  ctx.strokeStyle='rgba(64,200,255,0.7)';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(CX,CY);ctx.lineTo(CX+Math.sin(sweepRad)*R,CY-Math.cos(sweepRad)*R);ctx.stroke();
  ...
  if(selfData){
    contacts.forEach(c=>{
      ...
      const altFt=(c.alt_msl_m||0)*3.28084;if(altFt<altMinFt||altFt>altMaxFt)return;
      ...
      if(showTrails&&trail.length>1){for(let i=1;i<trail.length;i++){const alpha=(i/trail.length)*0.4*sweepFade;ctx.strokeStyle=color;ctx.globalAlpha=alpha;ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(trail[i-1].x,trail[i-1].y);ctx.lineTo(trail[i].x,trail[i].y);ctx.stroke();}ctx.globalAlpha=1;}
      ...
      ctx.fillStyle=color;ctx.strokeStyle=color;ctx.lineWidth=isSelected?2:1.5;ctx.beginPath();ctx.arc(pos.x,pos.y,isSelected?5:3,0,Math.PI*2);ctx.fill();
      ...
      ctx.globalAlpha=sweepFade;
      if(showLabels){const spd=Math.round((c.speed_ms||0)*1.944);ctx.fillStyle=isSelected?'#ffffff':color;ctx.font=(isSelected?'bold ':'')+' 10px Share Tech Mono';ctx.textAlign='left';ctx.fillText((c.name||c.id),pos.x+8,pos.y-4);const altFtR=Math.round(altFt/100)*100;ctx.fillStyle='rgba(160,200,220,0.45)';ctx.font='9px Share Tech Mono';ctx.fillText(altFtR+'ft '+spd+'kt',pos.x+8,pos.y+7);}
      ctx.globalAlpha=1;
    });
  }
  if(selfData){const hdgRad=(selfData.heading_deg||0)*Math.PI/180;ctx.save();ctx.translate(CX,CY);ctx.rotate(hdgRad);ctx.fillStyle='#40c8ff';ctx.strokeStyle='#40c8ff';ctx.lineWidth=1.5;ctx.beginPath();ctx.moveTo(0,-10);ctx.lineTo(-6,6);ctx.lineTo(0,2);ctx.lineTo(6,6);ctx.closePath();ctx.fill();ctx.strokeStyle='rgba(64,200,255,0.4)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(0,-10);ctx.lineTo(0,-30);ctx.stroke();ctx.restore();}
  ctx.restore();
  ctx.strokeStyle='rgba(64,200,255,0.2)';ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(CX,CY,R,0,Math.PI*2);ctx.stroke();
...
function updateSidebar(){
  if(selfData){document.getElementById('s-aircraft').textContent=(selfData.aircraft||'—').toUpperCase();document.getElementById('s-hdg').textContent=Math.round(selfData.heading_deg||0)+'°';document.getElementById('s-ias').textContent=Math.round((selfData.ias_ms||0)*1.944)+' kts';document.getElementById('s-alt').textContent=Math.round((selfData.alt_msl_m||0)*3.28084).toLocaleString()+' ft';}
  ...
  list.innerHTML=sorted.map(c=>{const color=iffColor(c.coalition);const dist=c.dist_m>=1000?(c.dist_m/1000).toFixed(1)+'km':Math.round(c.dist_m)+'m';const alt=Math.round((c.alt_msl_m||0)*3.28084/100)*100;const isThreat=c.coalition===2&&c.dist_m<20000;return'<div class="contact-row'+(c.id===selectedId?' selected':'')+(isThreat?' threat':'')+'" onclick="selectContact(\''+c.id+'\')">'+'<div class="iff-dot" style="background:'+color+'"></div>'+'<div class="contact-name">'+(c.name||c.id)+'</div>'+'<div class="contact-dist">'+dist+'</div>'+'<div class="contact-alt">'+alt+'ft</div>'+'</div>';}).join('');
}
function selectContact(id){
  ...
  if(c&&selectedId){
    ...
    panel.innerHTML='<div style="font-family:var(--hud);font-size:11px;color:'+color+';margin-bottom:8px">'+(c.name||c.id).toUpperCase()+'</div>'+'<div class="kv"><span class="kv-label">Type</span><span class="kv-value" style="font-size:11px">'+(c.type||'—')+'</span></div>'+'<div class="kv"><span class="kv-label">Category</span><span class="kv-value" style="font-size:11px">'+(c.category||'—')+'</span></div>'+'<div class="kv"><span class="kv-label">Distance</span><span class="kv-value" style="font-size:11px">'+dist+'</span></div>'+'<div class="kv"><span class="kv-label">Altitude</span><span class="kv-value" style="font-size:11px">'+Math.round((c.alt_msl_m||0)*3.28084).toLocaleString()+' ft</span></div>'+'<div class="kv"><span class="kv-label">Heading</span><span class="kv-value" style="font-size:11px">'+Math.round(c.heading_deg||0)+'°</span></div>'+'<div class="kv"><span class="kv-label">Coalition</span><span class="kv-value" style="font-size:11px;color:'+color+'">'+(c.coalition===1?'ALLIES':c.coalition===2?'ENEMY':'NEUTRAL')+'</span></div>';
